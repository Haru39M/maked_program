...
def search_and_save(apilogin, searchtag, min_score, range_num, directory):
    ...
    json_result = api.ranking_all(mode='male_r18',page=1,per_page=50,date=None)#ランキングの男性向けr18
    print(json_result.response[0].works[0].work.image_urls.large)
...
def main():
    ...
    apilogin  = login("harutowakayama3@gmail.com", "Harusamebiyori3") #ユーザ名とパスワード入力してログイン。login関数から返されたlogin情報をapiloginに格納
