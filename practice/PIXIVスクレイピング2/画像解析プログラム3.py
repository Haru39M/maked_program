...
while S < MAX:
    ...
    foldernamelist = os.listdir(path="./pixiv_images/bookmark/")
    foldername = foldernamelist[S]
    ...
    if not os.path.exists("./pixiv_images/bookmark/" + foldername + "/" + filename):
        tiled_color_img.save("パレット" + filename)
        shutil.move("./パレット"+filename,"./pixiv_images/bookmark/" + foldername,)
    S += 1
